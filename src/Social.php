...
class Social
{
    ...
    {
        if ($id !== null) {
            $social = execute_query("SELECT * from social
                where id = $id")->fetch_object("GiveToken\Social");
    ...
    public function save()
    {
        ...
        $social = execute_query("SELECT * from social
            ...
            and network = '$this->network'")->fetch_object("GiveToken\Social");
        foreach (get_object_vars($social) as $key => $value) {
            $hold->$key = $value;
        }
        if (!$hold->id) {
            ...
            $sql = "UPDATE social
                SET user_id = '$this->user_id', network = '$this->network', "
                . "url = '$this->url' "
                . "WHERE id = $hold->id";
            execute($sql);
        }
