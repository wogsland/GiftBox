...
class fakeSMTP{
	var $logFile = false;
	var $serverHello = 'fakeSMTP ESMTP PHP Mail Server Ready';
	...
	public function receive(){
		...
		while ($data = fgets(STDIN)){
			...
			else if (!$receivingData && preg_match('/^RCPT TO:\s?<(.*)>/i', $data, $match)){
				if (!$hasValidFrom){
			...
			else if (!$receivingData && preg_match('/^RSET$/i', trim($data))){
				$this->reply('250 2.0.0 Ok');
				$hasValidFrom = false;
				$hasValidTo = false;
			}
			else if (!$receivingData && preg_match('/^NOOP$/i', trim($data))){
				$this->reply('250 2.0.0 Ok');
			}
			else if (!$receivingData && preg_match('/^VRFY (.*)/i', trim($data), $match)){
				$this->reply('250 2.0.0 '.$match[1]);
			}
			else if (!$receivingData && preg_match('/^DATA/i', trim($data))){
				if (!$hasValidTo) {
			...
			else if (!$receivingData && preg_match('/^(HELO|EHLO)/i', $data)){
				$this->reply('250 HELO '.$this->mail['ipaddress']);
			}
			else if (!$receivingData && preg_match('/^QUIT/i', trim($data))){
				break;
			}
	...
	public function log($s){
		if ($this->logFile) file_put_contents($this->logFile, trim($s)."\n", FILE_APPEND);
	}
...
/* Optionally do something with $hp->mail(array) here */
