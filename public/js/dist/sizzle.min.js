...
window.matchMedia||(window.matchMedia=function(){var b=(window.styleMedia||window.media);if(!b){var c=document.createElement("style"),a=document.getElementsByTagName("script")[0],d=null;c.type="text/css";c.id="matchmediajs-test";a.parentNode.insertBefore(c,a);d=("getComputedStyle" in window)&&window.getComputedStyle(c,null)||c.currentStyle;b={matchMedium:function(e){var f="@media "+e+"{ #matchmediajs-test { width: 1px; } }";if(c.styleSheet){c.styleSheet.cssText=f}else{c.textContent=f}return d.width==="1px"}}}return function(e){return{matches:b.matchMedium(e||"all"),media:e||"all"}}}());(function(a){a.ajaxChimp={responses:{"We have sent you a confirmation email":0,"Please enter a value":1,"An email address must contain a single @":2,"The domain portion of the email address is invalid (the portion after the @: )":3,"The username portion of the email address is invalid (the portion before the @: )":4,"This email address looks fake or invalid. Please enter a real email address":5},translations:{en:null},init:function(b,c){a(b).ajaxChimp(c)}};a.fn.ajaxChimp=function(b){a(this).each(function(f,j){var h=a(j);var e=h.find("input[type=email]");var d=h.find("label[for="+e.attr("id")+"]");var g=a.extend({url:h.attr("action"),language:"en"},b);var c=g.url.replace("/post?","/post-json?").concat("&c=?");h.attr("novalidate","true");e.attr("name","EMAIL");h.submit(function(){function k(s){if(s.result==="success"){r="We have sent you a confirmation email";d.removeClass("error").addClass("valid");e.removeClass("error").addClass("valid")}else{e.removeClass("valid").addClass("error");d.removeClass("valid").addClass("error");var n=-1;var r;try{var q=s.msg.split(" - ",2);if(q[1]===undefined){r=s.msg}else{var o=parseInt(q[0],10);if(o.toString()===q[0]){n=q[0];r=q[1]}else{n=-1;r=s.msg}}}catch(p){n=-1;r=s.msg}}if(g.language!=="en"&&a.ajaxChimp.responses[r]&&a.ajaxChimp.translations&&a.ajaxChimp.translations[g.language]&&a.ajaxChimp.translations[g.language][a.ajaxChimp.responses[r]]){r=a.ajaxChimp.translations[g.language][a.ajaxChimp.responses[r]]}d.html(r);d.show(2000);if(g.callback){g.callback(s)}}var m={};var i=h.serializeArray();a.each(i,function(n,o){m[o.name]=o.value});a.ajax({url:c,data:m,success:k,dataType:"jsonp",error:function(o,n){console.log("mailchimp ajax submit error: "+n)}});var l="Submitting...";if(g.language!=="en"&&a.ajaxChimp.translations&&a.ajaxChimp.translations[g.language]&&a.ajaxChimp.translations[g.language]["submit"]){l=a.ajaxChimp.translations[g.language]["submit"]}d.html(l).show(2000);return false})});return this}})(jQuery);(function(b,c){b.Sizzle={Components:{},Controllers:{},Domain:{},Data:{},Util:{}};var a=c(Sizzle);Sizzle.on=a.on.bind(a);Sizzle.off=a.off.bind(a);Sizzle.trigger=a.trigger.bind(a);c(function(){Sizzle.trigger({type:"bootstrap"})})}(this,jQuery));(function(b){function a(c){this.$el=$(c);this.$el.data("Form",this);this._$submit=this.$el.find('[type="submit"]');this._$feedback=this.$el.find(".Form__feedback");this._$title=this.$el.find(".Form__title");this._registerEvents()}a.prototype={_registerEvents:function(){this.$el.on("change.Form",".Form__input",function(c){this._updateLabel(c.currentTarget)}.bind(this));this.$el.find(".Form__input").change()},_updateLabel:function(d){var c=$(d).siblings(".Form__label");if(d.value){c.addClass("Form__label--hidden")}else{c.removeClass("Form__label--hidden")}},validate:function(){if(this.$el.find('[data-validation~="required"]').filter(function(d,c){return c.value===""}).length){this.error("All fields required");return false}if(this.$el.find('[data-validation~="email"]').filter(function(d,c){return !c.value.match(/^[^\@]+@[^\@]+\.[^\@\.]+$/)}).length){this.error("Email appears invalid");return false}return true},disableSubmit:function(){this._$submit.attr("disabled",true)},enableSubmit:function(){this._$submit.removeAttr("disabled")},title:function(c){if(typeof c==="string"){this._$title.html(c)}return this._$title.html()},get:function(c){return this.$el.find("#"+c+', [name="'+c+'"]')[0]},set:function(c,d){this.get(c).value=d},error:function(c){this._$feedback.text(c);this.$el.addClass("Form--error")},clearError:function(){this._$feedback.text("");this.$el.removeClass("Form--error")}};b.Components.Form=a}(Sizzle));(function(d){var a={left:false},e=$(document),c=$("body");e.on("mousedown.left_click",function(){a.left=true});e.on("mouseup.left_click",function(){a.left=false});function b(h,g,f,i){$(h).data("slider",this);this._track=h;this._knob=h.children[0];this._label=this._knob.children[0];this._input=h.children[1];this._min=parseFloat(g||h.dataset.min||0);this._max=parseFloat(f||h.dataset.max||100);this.$this=$(this);this.value(i||h.dataset.value||g);this._is_dragging=false;this._register_events()}b.prototype={value:function(f){if(typeof f!=="undefined"){this._value=parseFloat((f-this._min)/this.range());this._update();this.$this.trigger("change",this.value())}return this.range()*this._value+this._min},range:function(){return this._max-this._min},drag:function(){this._is_dragging=true;c.addClass("App--dragging")},drop:function(){this._is_dragging=false;c.removeClass("App--dragging")},is_dragging:function(){return this._is_dragging},_update:function(){this._knob.style.left=this._value*100+"%";this._input.value=Math.round(this.value());this._label.innerHTML=this._input.value},_update_value:function(f){var g=this._track.getBoundingClientRect(),i=g.left+document.documentElement.scrollLeft,h=g.width;this.value(this.range()*(Math.min(1,Math.max(0,(f-i)/h)))+this._min);this._update()},_register_events:function(){$(this._track).on("mousedown.drag",function(f){this.drag();this._update_value(f.clientX);f.preventDefault()}.bind(this));e.on("mouseup.drop",this.drop.bind(this)).on("mousemove.drag",function(f){if(this.is_dragging()&&a.left){this._update_value(f.clientX)}}.bind(this))}};d.Components.Slider=b}(Sizzle));(function(d){var b={};d.Controllers.create=function(f,e){b[f]=b[f]||[];b[f].push(e)};function a(){for(var e in b){c(e)}}function c(f){var e=document.getElementById(f);if(e&&b[f]){b[f].forEach(function(g){g(e)})}}d.on("bootstrap.controllers",a)}(Sizzle));(function(c,b){function a(d){this._$el=b(d);this._$plan_dialog=b("#plan-dialog");this._$contact_dialog=b("#contact-dialog");this._slider=new c.Components.Slider(this._$el.find(".Slider")[0]);this._plans=this._$el.find(".Plan");this._recommended=null;this._existingUser=false;this._planForm=new c.Components.Form(this._$plan_dialog.find(".Form")[0]);this._contactForm=new c.Components.Form(this._$contact_dialog.find(".Form")[0]);this.update_recommendation(this._slider.value());this._register_events()}a.prototype={_register_events:function(){this._slider.$this.on("change",function(g,f){this.update_recommendation(f)}.bind(this));var d=b(document);d.on("click.pricing",".Plan__callToAction",function(f){if(f.currentTarget.dataset.plan){this._planForm.set("plan",f.currentTarget.dataset.plan);this._planForm.set("billing",f.currentTarget.dataset.billing);this._planForm.clearError();this._planForm.title(b(f.currentTarget).parents(".Plan").find(".Plan__name").text()+' Plan <span class="Form__subtitle">'+f.currentTarget.innerHTML.replace(/<div>.+<\/div>/,"")+"</span>");this._$plan_dialog.modal()}else{this._contactForm.clearError();this._$contact_dialog.modal()}return false}.bind(this));this._planForm.$el.on("submit.pricing",function(){if(this._planForm.validate()){this._planForm.disableSubmit();this._planForm.clearError();Stripe.card.createToken(this._planForm.$el,this._onStripeResponse.bind(this))}return false}.bind(this));this._contactForm.$el.on("submit.pricing",function(){if(this._contactForm.validate()){this._contactForm.disableSubmit();this._contactForm.clearError();b.ajax({url:"/ajax/sendemail",data:this._contactForm.$el.serialize(),type:"post"}).done(this._redirectToThankYou("enterprise")).fail(this._onProcessFailure.bind(this))}return false}.bind(this))},_validate_form:function(d){if(d.$el.find('[data-validation~="required"]').filter(function(f,e){return e.value===""}).length){d.error("All fields required");return false}if(d.$el.find('[data-validation~="email"]').filter(function(f,e){return !e.value.match(/^[^\@]+@[^\@]+\.[^\@\.]+$/)}).length){d.error("Email appears invalid");return false}return true},update_recommendation:function(d){var e=this._plans[0];this._plans.each(function(f,g){if(g.dataset.min<=d){e=g}}.bind(this));this.recommend(e)},recommend:function(d){this._recommended=b(d);this._plans.removeClass("Plan--recommended");this._recommended.addClass("Plan--recommended")},_onStripeResponse:function(d,e){if(e.error){this._onProcessFailure(e.error.message)}else{this._signupUser(d,e)}},_signupUser:function(d,e){this._planForm.set("stripeToken",e.id);var f=this._planForm.get("email").value;b.ajax({url:"/ajax/signup",data:{signup_email:f},type:"post",dataType:"json"}).then(function(g){if(g.message=="The email address "+f+" has already been registered."){this._existingUser=true}}.bind(this)).then(this._upgradePlan.bind(this)).then(this._redirectToThankYou("signup").bind(this)).fail(this._onProcessFailure.bind(this))},_upgradePlan:function(){return b.ajax({url:"/ajax/upgrade",data:this._planForm.$el.serialize(),type:"post",dataType:"json"})},_redirectToThankYou:function(d){return function(){if("signup"==d&&this._existingUser){window.location="/thankyou?action=upgrade"}else{window.location="/thankyou?action="+d}}},_onProcessFailure:function(d){d=typeof d==="string"?d:"There was a problem processing your request. Please try again later.";this._planForm.error(d);this._planForm.enableSubmit()}};c.Controllers.create("pricing-page",function(e){var d=new a(e)})}(Sizzle,jQuery));jQuery(window).load(function(){jQuery(".status").fadeOut();jQuery(".preloader").delay(1000).fadeOut("slow")});$(document).ready(function(){$window=$(window);$('section[data-type="background"]').each(function(){var a=$(this);$(window).scroll(function(){var b=-($window.scrollTop()/a.data("speed"));var c="50% "+b+"px";a.css({backgroundPosition:c})})})});document.createElement("section");$(".mailchimp").ajaxChimp({callback:mailchimpCallback,url:"http://webdesign7.us6.list-manage.com/subscribe/post?u=9445a2e155b82208d73433060&amp;id=16dc80e353"});function mailchimpCallback(a){if(a.result==="success"){$(".subscription-success").html('<i class="icon_check_alt2"></i><br/>'+a.msg).fadeIn(1000);$(".subscription-error").fadeOut(500)}else{if(a.result==="error"){$(".subscription-error").html('<i class="icon_close_alt2"></i><br/>'+a.msg).fadeIn(1000)}}}var scrollAnimationTime=1200,scrollAnimation="easeInOutExpo";$("a.scrollto").bind("click.smoothscroll",function(a){a.preventDefault();var b=this.hash;$("html, body").stop().animate({scrollTop:$(b).offset().top},scrollAnimationTime,scrollAnimation,function(){window.location.hash=b})});wow=new WOW({mobile:false});wow.init();$("#subscribe").submit(function(f){f.preventDefault();var a=$("#subscriber-email").val();var b="email="+a;function c(g){var e=new RegExp(/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i);return e.test(g)}if(c(a)){var d=event.target;$(d).addClass("disable-clicks");$.ajax({type:"POST",url:"subscribe/subscribe.php",data:b,success:function(){$(".subscription-success").fadeIn(1000);$(".subscription-error").fadeOut(500);$(".hide-after").fadeOut(500)},complete:function(){$(d).removeClass("disable-clicks")}})}else{$(".subscription-error").fadeIn(1000)}return false});jQuery(".tooltips").tooltip({container:"body"});$(".expand-form").simpleexpand({defaultTarget:".expanded-contact-form"});$(window).stellar({horizontalScrolling:false});if(navigator.userAgent.match(/IEMobile\/10\.0/)){var msViewportStyle=document.createElement("style");msViewportStyle.appendChild(document.createTextNode("@-ms-viewport{width:auto!important}"));document.querySelector("head").appendChild(msViewportStyle)}$(document).ready(function(){$("#dataTable #checkall").click(function(){if($("#mytable #checkall").is(":checked")){$("#mytable input[type=checkbox]").each(function(){$(this).prop("checked",true)})}else{$("#mytable input[type=checkbox]").each(function(){$(this).prop("checked",false)})}})});window.fbAsyncInit=function(){FB.init({appId:"1498055593756885",cookie:true,xfbml:true,version:"v2.0"})};(function(e,a,f){var c,b=e.getElementsByTagName(a)[0];if(e.getElementById(f)){return}c=e.createElement(a);c.id=f;c.src="//connect.facebook.net/en_US/sdk.js";b.parentNode.insertBefore(c,b)}(document,"script","facebook-jssdk"));var confirmOnPageExit=function(b){b=b||window.event;var a="Any unsaved changes will be lost!";if(b){b.returnValue=a}return a};function saved(){window.onbeforeunload=null}function openStatus(b,a){$("#status-dialog").dialog("option","title",b);$("#status-text").text(a);$("#status-dialog").dialog("open")}function closeStatus(){$("#status-dialog").dialog("close")}function setStatus(a){$("#status-text").text(a)}function getSession(){var a=null;$.ajax({url:"/ajax/get_session",async:false}).done(function(c,d,b){a=c});return a}function logout(){$("#logout-button").addClass("disable-clicks");$.post("/ajax/logout",function(b,c,a){if(b.status==="SUCCESS"){if(b.login_type==="FACEBOOK"){FB.getLoginStatus(function(d){if(d.status==="connected"){FB.logout(function(e){})}})}document.location.href=b.app_root}else{if(b.status==="ERROR"){alert(b.message)}else{alert("Unknown return status: "+b.status)}}}).fail(function(){alert("Logout failed.")}).always(function(){$("#logout-button").removeClass("disable-clicks")})}function showStatus(b,a){$(b).removeClass("red-text");$(b).text(a)}function showError(b,a){$(b).addClass("red-text");$(b).text(a)};